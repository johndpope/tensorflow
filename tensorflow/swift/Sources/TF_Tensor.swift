...
extension tf{
    ...
    public class func NewTensor(dt: TF_DataType, _ dims: UnsafePointer<Int64>!, _ num_dims: Int32, _ data: UnsafeMutableRawPointer!, _ len: Int, _ deallocator: (@convention(c) (UnsafeMutableRawPointer?, Int, UnsafeMutableRawPointer?) -> Swift.Void)!, _ deallocator_arg: UnsafeMutableRawPointer!) -> TF_Tensor!{
    ...
    // Destroy a tensor.
    public class func DeleteTensor(_ pointer:TF_Tensor!){
        return TF_DeleteTensor(pointer)
    }
    
    // Return the type of a tensor element.
    public class func TensorType(_ pointer:TF_Tensor!) -> TF_DataType{
        return TF_TensorType(pointer)
    }
    
    // Return the number of dimensions that the tensor has.
    public class func NumDims(_ pointer: TF_Tensor!) -> Int32{
        return TF_NumDims(pointer)
    }
    
    // Return the length of the tensor in the "dim_index" dimension.
    // REQUIRES: 0 <= dim_index < TF_NumDims(tensor)
    public class func Dim(_ tensor: TF_Tensor!, _ dim_index: Int32) -> Int64{
        return TF_Dim(tensor,dim_index)
    }
    
    // Return the size of the underlying data in bytes.
    public class func TensorByteSize(_ pointer: TF_Tensor!) -> Int{
        return  TF_TensorByteSize(pointer)
    }
    
    // Return a pointer to the underlying data buffer.
    public class func TensorData(_ pointer: TF_Tensor!) -> UnsafeMutableRawPointer!{
